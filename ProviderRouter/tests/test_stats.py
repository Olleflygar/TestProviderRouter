...
from nygen_router import ApiProtocol, MetricsEvent, aggregate_stats
...
def test_a_provider_with_only_regular_events_leaves_its_streaming_fields_empty() -> None:
    ...
    stats = aggregate_stats(events, ["provider_a"])["provider_a"]
...
def test_a_completed_stream_with_no_chunk_to_time_reports_no_average() -> None:
    ...
    stats = aggregate_stats([_event(stream=True, latency_ms=None)], ["provider_a"])["provider_a"]
...
def test_a_provider_with_no_events_still_gets_an_entry() -> None:
    ...
    result = aggregate_stats([_event("provider_a")], ["provider_a", "brand_new"])
...
def test_events_for_a_provider_not_asked_about_are_ignored() -> None:
    ...
    result = aggregate_stats(events, ["provider_a"])
...
def test_error_tallies_count_each_category_separately() -> None:
    events = [
        ...
        _failure(stream=True, error_type=ErrorCategory.STREAM_INTERRUPTED.value),
    ]
...
def test_regular_and_streaming_events_never_influence_each_other() -> None:
    ...
    assert stats.regular_avg_latency_ms == pytest.approx(200.0)
    assert stats.streaming_success_rate == pytest.approx(0.0)
    assert stats.streaming_avg_ttft_ms is None


def test_weight_fn_scales_every_count_it_touches() -> None:
    events = [_event(latency_ms=100.0), _failure(latency_ms=10.0), _event(stream=True)]

    unweighted = aggregate_stats(events, ["provider_a"])["provider_a"]
    weighted = aggregate_stats(events, ["provider_a"], weight_fn=lambda event: 2.0)["provider_a"]
    ...
    assert weighted.streaming_attempt_count == pytest.approx(unweighted.streaming_attempt_count * 2)
    assert weighted.streaming_success_count == pytest.approx(unweighted.streaming_success_count * 2)
...
def test_weight_fn_receives_each_event_exactly_once() -> None:
    ...
    aggregate_stats(events, ["provider_a"], weight_fn=weight)
...
def test_every_requested_provider_is_reported_once_in_the_order_asked_for() -> None:
    ...
    result = aggregate_stats(events, ["provider_a", "provider_b", "provider_c"])
