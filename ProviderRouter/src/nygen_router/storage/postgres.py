...
from nygen_router.errors import ConfigError, ErrorCategory
...
class PostgresMetricsStore:
    ...
    Optional: install with pip install "nygen-router[postgres]". Works against
    ...
    it deliberately through nygen-router storage create --backend postgres, or
        ...
        *,
        ...
        if not available:
            logger.warning(
                ...
                'pip install "nygen-router[postgres]", or pass a different metrics_store '
                "to ProviderRouter."
            )
