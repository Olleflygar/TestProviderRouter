...
def inspect_postgres_database(url: str) -> PostgresInspection:
    ...
    from llm_provider_router.storage.postgres import _inspect_connection, redact_postgres_url
...
def _require_duckdb() -> Any:
    try:
        ...
        raise StorageDependencyError(
            ...
            'pip install "llm-provider-router[duckdb]".'
        ) from exc


def _inspect_sqlite_path(path: Path) -> SchemaReport:
    from llm_provider_router.storage.sqlite import _inspect_connection
...
def _inspect_duckdb_path(path: Path) -> SchemaReport:
    from llm_provider_router.storage.duckdb import _inspect_connection
...
def _temporary_database_path(destination: Path) -> Path:
    descriptor, name = tempfile.mkstemp(
        ...
        prefix=".llm-provider-router-create-",
